...
void list_add_ascending_by_blocksize(list_t *l, block_t *newblk){
  ...
  if(l->head == NULL){
    ...
    prev = curr = l->head;
    currBlockSize = curr->blk->end - curr->blk->start + 1;
    if(curr->next == NULL){  
      if(newblk_size >= currBlockSize){  
        newNode->next = l->head;
        l->head = newNode;
      }
      else{   
      ...
      if(newblk_size >= currBlockSize){  
        ...
        while(curr != NULL && newblk_size <= currBlockSize){
          prev = curr;
          curr = curr->next;
          if(curr != NULL){
            currBlockSize = curr->blk->end - curr->blk->start;
          }  
        }
        prev->next = newNode;
        newNode->next = curr;
